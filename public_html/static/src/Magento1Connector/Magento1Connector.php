...
class Magento1Connector {
   ...
   * @return array
   */
  public function getAttributeValuesRaw(string $attributeName, bool $useCache = false, string $lang = AppConfig::DEFAULT_LANG): array {

    if ($useCache) {
      $options = CacheHelper::getAttributeValuesRaw($attributeName, $lang);
      if ($options) {
        return $options;
      }
    }
